...
public class BackgroundEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (foldoutNoise)
        {
            myTarget.CustomNoiseGenerator = EditorGUILayout.BeginToggleGroup("Noise Generator", myTarget.CustomNoiseGenerator);
            {
                EditorGUI.indentLevel++;
                myTarget.NoiseGenerator = (SS_NoiseGenerator)EditorGUILayout.EnumPopup(myTarget.NoiseGenerator);
                EditorGUI.indentLevel--;
            }
            EditorGUILayout.EndToggleGroup();
        ...
        if (foldoutAppearance)
        {
            ...
            {
                EditorGUILayout.LabelField("Tiling Count:", GUILayout.Width(labelWidth));
                myTarget.TilingCount = EditorGUILayout.IntSlider(myTarget.TilingCount, 1, 2);
            }
